...
QData Vadd___024root___change_request(Vadd___024root* vlSelf);
#ifdef VL_DEBUG
...
static void _eval_initial_loop(Vadd__Syms* __restrict vlSymsp) {
    ...
    do {
        ...
        if (VL_UNLIKELY(++__VclockLoop > 100)) {
            ...
            Verilated::debug(1);
            __Vchange = Vadd___024root___change_request(&(vlSymsp->TOP));
...
static void traceInit(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    tracep->scopeEscape(' ');
    Vadd___024root__traceInitTop(vlSelf, tracep);
    tracep->scopeEscape('.');
}
...
void Vadd::trace(VerilatedVcdC* tfp, int, int) {
    ...
    Vadd___024root__traceRegister(&(vlSymsp->TOP), tfp->spTrace());
}
